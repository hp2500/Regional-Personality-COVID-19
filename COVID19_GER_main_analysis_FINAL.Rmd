...
rbind(ger_means, ger_sd) %>% select(-ewz) %>% t() %>% round(3) %>% as.data.frame() %>%
  write_csv('/Users/hp2500/Google Drive/STUDY/Columbia/Research/Corona/Delivery/ger_descriptives.csv')

df_ger_socdist$kreis %>% unique() %>% length()
df_ger_prev$kreis %>% unique() %>% length()

df_ger_desc %>% summarise_all(sd)
...
df_ger_prev %>% group_by(date) %>% summarize(sum(cumcase))
```
