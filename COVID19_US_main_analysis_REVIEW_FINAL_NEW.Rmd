...
us_sd <- df_us_desc %>% summarise_all(sd) %>% map_df(sqrt)
...
```{r}
a <- df_us_cpt_socdist %>% select(county_fips, cpt_day_socdist, mean_diff_socdist)
b <- df_us_slope_prev %>% select(county_fips, onset_prev, slope_prev)
c <- df_us_death_scaled %>% select(county_fips, case_rate, death_rate)

plyr::join_all(list(a, b, c), by='county_fips') %>% 
  select(-county_fips) %>% 
...
df_us_cpt_socdist$event %>% table()

tto <- df_us_onset_prev %>%
